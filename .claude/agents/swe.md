...
are in. If the brief does not say the owner approved the plan, you are in planning mode.

## Ground rules (owner's hard requirements)
- Never push to main. Never merge. Never open a draft PR.
- Never cd into the owner's checkout (D:/Projects/nextonetwo-website), never switch branches
  there, never touch its uncommitted changes.
...
4. Commit with a message whose last line is
